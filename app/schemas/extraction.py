...
class PlaceLocation(BaseModel):
    ...
    confidence: int = Field(
        ..., ge=0, le=100, json_schema_extra={"example": 96}, description="Confidence percentage score (0-100)"
    )
    address: Optional[str] = Field(
        None, json_schema_extra={"example": "68 Fukakusa Yabunouchicho, Fushimi Ward, Kyoto, 612-0882, Japan"}
    )
    ...
    verified: bool = Field(
        default=False, description="Whether the location was successfully verified by Google Places API"
    )
...
class ImageExtractionResponse(BaseModel):
    ...
    destination: Optional[str] = Field(
        ..., json_schema_extra={"example": "Kyoto"}, description="Primary overall destination or region identified"
    )

...
class TravelContent(BaseModel):
    ...
    source_type: str = Field(..., json_schema_extra={"example": "social_post"}, description="Source type: image, video, text, social_post")
    caption: Optional[str] = Field(None, json_schema_extra={"example": "Exploring Shibuya crossing in Tokyo!"}, description="Text caption or post content")
    ...
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Metadata key-value attributes (URL, author, platform)")
...
class UniversalExtractionRequest(BaseModel):
    ...
    context: Optional[str] = Field(
        None,
        json_schema_extra={"example": "Japan trip"},
        description="Optional location context or region hint"
    )

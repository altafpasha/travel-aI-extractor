...
class ExtractionService:
    ...
    async def process_travel_content(self, content: TravelContent) -> ImageExtractionResponse:
        ...
        if content.source_type == "text" or (not content.frames and content.caption):
            return await self.process_text_extraction(
                ...
                context=content.metadata.get("context_hint")
            )

        if content.source_type == "image" and content.frames:
            return await self.process_image_extraction(
                image_bytes=content.frames[0],
                filename=content.metadata.get("filename", "image_source.jpg")
            )

        if content.source_type == "video" and content.frames:
            return await self.process_video_extraction(
                video_bytes=content.frames[0],
                filename=content.metadata.get("filename", "video_source.mp4")
            )
    ...
    async def process_text_extraction(
        ...
        raw_extraction = await self.gemini_service.extract_places_from_text(
            text=text,
            context=context
        )
    ...
    async def process_video_extraction(
        ...
        logger.info(f"Cache MISS: Executing multi-modal video extraction pipeline for '{filename}' ({len(video_bytes)} bytes)...")
    ...
    async def _finalize_extraction_pipeline(
        ...
        if raw_places:
            verification_tasks = [
                self.places_service.verify_and_enrich_place(place, text_context=text_context)

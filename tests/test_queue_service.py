...
async def test_enqueue_async_job_and_poll_status(client):
    ...
    payload = {"source_type": "text", "content": "Exploring Kyoto Fushimi Inari Shrine", "context": "Async Test"}

    # Enqueue
    res_enqueue = await client.post(f"{settings.API_V1_STR}/extract/async", json=payload, headers=headers)
    ...
    # Poll
    res_poll = await client.get(f"{settings.API_V1_STR}/jobs/{job_id}", headers=headers)
...
async def test_get_nonexistent_job_status_returns_404(client):
    ...
    settings = get_settings()
    headers = {"X-API-Key": settings.API_KEY}
    res = await client.get(f"{settings.API_V1_STR}/jobs/nonexistent_job_999999", headers=headers)
